...
def solution(num_buns, num_required):
    combs_bun_complement = list(itertools.combinations([i for i in range(num_buns)],num_buns-num_required+1))
    num_keys = len(combs_bun_complement)
    keys_for_buns = [set() for i in range(num_buns)]

    combs_bun_complement = sorted([set(c) for c in combs_bun_complement])
    
    for i in range(num_keys):
        for j in range(num_buns):
            if j in combs_bun_complement[i]:
                keys_for_buns[j].add(i)
            
    for i in range(num_buns):
        keys_for_buns[i] = sorted(set(keys_for_buns[i]))
    
    keys_for_buns = sorted(keys_for_buns)
    
    return keys_for_buns

num_buns, num_required = 10,5
print(solution(num_buns, num_required))
